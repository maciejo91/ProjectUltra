import { createHourOffset, createDateTimeOffset } from '@/utils/mockDataHelpers'

export const calendarEventTypes = [
  { value: 'test-drive', label: 'Test Drive', category: 'negotiation' },
  { value: 'appointment', label: 'Dealership Visit', category: 'negotiation' },
  { value: 'offsite', label: 'Offsite Visit', category: 'negotiation' },
  { value: 'workshop', label: 'Workshop', category: 'negotiation' },
  { value: 'call', label: 'Call', category: 'negotiation' },
  { value: 'delivery', label: 'Delivery', category: 'negotiation' },
  { value: 'recall', label: 'Recall', category: 'negotiation' },
  { value: 'absence', label: 'Absence', category: 'reminder' },
  { value: 'leave', label: 'Leave', category: 'reminder' },
  { value: 'marketing', label: 'Marketing Event', category: 'reminder' },
  { value: 'memo', label: 'Memo', category: 'reminder' },
  { value: 'training', label: 'Training', category: 'reminder' },
  { value: 'meeting', label: 'Meeting', category: 'negotiation' },
  { value: 'other', label: 'Other', category: 'reminder' }
]

export const calendarDealerships = [
  { id: 1, name: 'Bari' },
  { id: 2, name: 'Firenze' },
  { id: 3, name: 'Milano' },
  { id: 4, name: 'Roma' },
  { id: 5, name: 'Trento' }
]

export const calendarTeams = [
  { id: 1, name: 'BDC', dealershipId: 3, dealership: 'Milano' },
  { id: 2, name: 'Call Center', dealershipId: 3, dealership: 'Milano' },
  { id: 3, name: 'General', dealershipId: 3, dealership: 'Milano' },
  { id: 4, name: 'Service', dealershipId: 2, dealership: 'Firenze' },
  { id: 5, name: 'Sales (New)', dealershipId: 3, dealership: 'Milano' },
  { id: 6, name: 'Sales (Rent)', dealershipId: 2, dealership: 'Firenze' },
  { id: 7, name: 'Sales (Used)', dealershipId: 2, dealership: 'Firenze' },
  { id: 8, name: 'Sales (New)', dealershipId: 2, dealership: 'Firenze' },
  { id: 9, name: 'Service', dealershipId: 1, dealership: 'Bari' },
  { id: 10, name: 'Sales (New)', dealershipId: 1, dealership: 'Bari' },
  { id: 11, name: 'BDC', dealershipId: 4, dealership: 'Roma' },
  { id: 12, name: 'Sales (New)', dealershipId: 4, dealership: 'Roma' },
  { id: 13, name: 'Service', dealershipId: 4, dealership: 'Roma' },
  { id: 14, name: 'Sales (New)', dealershipId: 5, dealership: 'Trento' },
  { id: 15, name: 'Service', dealershipId: 5, dealership: 'Trento' },
  { id: 16, name: 'General', dealershipId: 2, dealership: 'Firenze' },
  { id: 17, name: 'General', dealershipId: 1, dealership: 'Bari' },
  { id: 18, name: 'General', dealershipId: 4, dealership: 'Roma' },
  { id: 19, name: 'General', dealershipId: 5, dealership: 'Trento' }
]

export const mockCalendarEvents = [
  {
    id: 4,
    title: 'Meeting - Oliviero Bruno',
    start: '2025-12-20T11:00:00',
    end: '2025-12-20T12:00:00',
    type: 'meeting',
    customer: 'Oliviero Bruno',
    customerId: 5,
    opportunityId: 2,
    vehicle: 'Porsche Taycan',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Milano',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 5,
    title: 'Test Drive - Elisa Costa',
    start: '2025-12-21T13:00:00',
    end: '2025-12-21T14:00:00',
    type: 'test-drive',
    customer: 'Elisa Costa',
    customerId: 21,
    opportunityId: 1,
    vehicle: 'Audi e-tron GT',
    assignee: 'Matteo Greco',
    assigneeId: 1,
    dealership: 'Firenze',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 6,
    title: 'Dealership Visit - Sofia Bellini',
    start: '2025-12-22T10:00:00',
    end: '2025-12-22T11:00:00',
    type: 'appointment',
    customer: 'Sofia Bellini',
    customerId: 6,
    vehicle: 'BMW iX',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Roma',
    team: 'Sales (New)',
    status: 'cancelled',
    noShowCount: 0
  },
  {
    id: 7,
    title: 'Follow-up Call - Luca De Santis',
    start: '2025-12-23T15:00:00',
    end: '2025-12-23T15:30:00',
    type: 'call',
    customer: 'Luca De Santis',
    customerId: 1,
    leadId: 1,
    assignee: 'Matteo Alpino',
    assigneeId: 1,
    dealership: 'Firenze',
    team: 'BDC',
    status: 'no-show',
    noShowCount: 1
  },
  {
    id: 8,
    title: 'Test Drive - Chiara Moretti',
    start: '2025-12-26T09:00:00',
    end: '2025-12-26T10:00:00',
    type: 'test-drive',
    customer: 'Chiara Moretti',
    customerId: 2,
    vehicle: 'Mercedes-Benz EQS',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Milano',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0
  },
  {
    id: 9,
    title: 'Meeting - Oliviero Bruno',
    start: '2025-12-27T14:00:00',
    end: '2025-12-27T15:30:00',
    type: 'meeting',
    customer: 'Oliviero Bruno',
    customerId: 5,
    vehicle: 'Porsche Taycan',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Milano',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0
  },
  {
    id: 10,
    title: 'Dealership Visit - Elisa Costa',
    start: '2025-12-28T11:00:00',
    end: '2025-12-28T12:00:00',
    type: 'appointment',
    customer: 'Elisa Costa',
    customerId: 21,
    opportunityId: 1,
    vehicle: 'Audi e-tron GT',
    assignee: 'Matteo Greco',
    assigneeId: 1,
    dealership: 'Firenze',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 11,
    title: 'Workshop - Vehicle Inspection',
    start: '2025-12-29T09:00:00',
    end: '2025-12-29T12:00:00',
    type: 'workshop',
    customer: 'Luca De Santis',
    customerId: 1,
    leadId: 1,
    vehicle: 'Audi A6 Allroad',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Firenze',
    team: 'Service',
    status: 'confirmed',
    noShowCount: 0
  },
  {
    id: 12,
    title: 'Delivery - BMW iX',
    start: '2025-12-30T14:00:00',
    end: '2025-12-30T15:00:00',
    type: 'delivery',
    customer: 'Sofia Bellini',
    customerId: 6,
    vehicle: 'BMW iX',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Roma',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0
  },
  {
    id: 13,
    title: 'Training - New CRM Features',
    start: '2026-01-02T10:00:00',
    end: '2026-01-02T12:00:00',
    type: 'training',
    assignee: 'Matteo Alpino',
    assigneeId: 1,
    dealership: 'Milano',
    team: 'General',
    status: 'confirmed'
  },
  {
    id: 14,
    title: 'Marketing Event - New Year Sale Launch',
    start: '2026-01-03T09:00:00',
    end: '2026-01-03T18:00:00',
    type: 'marketing',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Milano',
    team: 'General',
    status: 'confirmed'
  },
  {
    id: 15,
    title: 'Offsite Visit - Matteo Bianchi',
    start: '2026-01-06T11:00:00',
    end: '2026-01-06T12:30:00',
    type: 'offsite',
    customer: 'Matteo Bianchi',
    customerId: 3,
    vehicle: 'Mercedes-Benz EQS',
    assignee: 'Natalia Santoro',
    assigneeId: 4,
    dealership: 'Bari',
    team: 'Sales (New)',
    status: 'confirmed'
  },
  {
    id: 16,
    title: 'Leave - Winter Vacation',
    start: '2026-01-07T00:00:00',
    end: '2026-01-10T23:59:59',
    type: 'leave',
    assignee: 'Sara Marino',
    assigneeId: 2,
    team: 'Sales (New)',
    status: 'confirmed'
  },
  {
    id: 17,
    title: 'Call - Follow-up Elisa Costa',
    start: '2026-01-08T10:00:00',
    end: '2026-01-08T10:30:00',
    type: 'call',
    customer: 'Elisa Costa',
    customerId: 21,
    assignee: 'Matteo Greco',
    assigneeId: 1,
    dealership: 'Firenze',
    team: 'BDC',
    status: 'confirmed'
  },
  {
    id: 18,
    title: 'Test Drive - Marco Rossi',
    start: '2026-01-09T15:00:00',
    end: '2026-01-09T16:00:00',
    type: 'test-drive',
    customer: 'Marco Rossi',
    vehicle: 'Volkswagen ID.4',
    assignee: 'Natalia Santoro',
    assigneeId: 4,
    dealership: 'Trento',
    team: 'Sales (New)',
    status: 'confirmed'
  },
  {
    id: 19,
    title: 'Memo - Q1 Targets Review',
    start: '2026-01-13T09:00:00',
    end: '2026-01-13T09:30:00',
    type: 'memo',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    team: 'General',
    status: 'confirmed'
  },
  {
    id: 20,
    title: 'Recall - Safety Check',
    start: '2026-01-15T08:00:00',
    end: '2026-01-15T17:00:00',
    type: 'recall',
    vehicle: 'Various Models',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Roma',
    team: 'Service',
    status: 'confirmed'
  },
  // Test-drive events for showcasing carousel - Customers 1-20
  {
    id: 25,
    title: 'Test Drive - Luca De Santis',
    start: (() => {
      const date = new Date()
      date.setDate(date.getDate() - 5)
      date.setHours(14, 0, 0, 0)
      return date.toISOString()
    })(),
    end: (() => {
      const date = new Date()
      date.setDate(date.getDate() - 5)
      date.setHours(15, 0, 0, 0)
      return date.toISOString()
    })(),
    type: 'test-drive',
    customer: 'Luca De Santis',
    customerId: 1,
    leadId: 1,
    opportunityId: 3,
    vehicle: 'Audi Q5',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Milan',
    team: 'Sales (New)',
    status: 'confirmed',
    bookedAt: createHourOffset(-28),
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 26,
    title: 'Test Drive - Matteo Bianchi',
    start: (() => {
      const date = new Date()
      date.setDate(date.getDate() - 3)
      date.setHours(10, 0, 0, 0)
      return date.toISOString()
    })(),
    end: (() => {
      const date = new Date()
      date.setDate(date.getDate() - 3)
      date.setHours(11, 0, 0, 0)
      return date.toISOString()
    })(),
    type: 'test-drive',
    customer: 'Matteo Bianchi',
    customerId: 3,
    vehicle: 'BMW M4 Coupe',
    assignee: 'Sara Marino',
    assigneeId: 2,
    dealership: 'Berlin',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0,
    previousAppointmentId: null
  },
  // Feb 23 - March 1 2026 (current week view)
  {
    id: 30,
    title: 'Q3 campaign kick-off',
    start: '2026-02-23T09:00:00',
    end: '2026-02-23T10:00:00',
    type: 'meeting',
    dealership: 'Milano',
    location: 'Meeting room A',
    assignee: 'Sara Marino',
    assigneeId: 2,
    team: 'General',
    status: 'confirmed',
  },
  {
    id: 31,
    title: 'XC40 test drive - Elisa Costa',
    start: '2026-02-23T10:00:00',
    end: '2026-02-23T11:00:00',
    type: 'test-drive',
    customer: 'Elisa Costa',
    customerId: 21,
    vehicle: 'Volvo XC40',
    dealership: 'Main showroom',
    assignee: 'Matteo Alpino',
    assigneeId: 1,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 32,
    title: 'Recall appointment - brake pads',
    start: '2026-02-23T11:30:00',
    end: '2026-02-23T12:30:00',
    type: 'recall',
    vehicle: 'Audi A6',
    dealership: 'Service bay 3',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    team: 'Service',
    status: 'confirmed',
  },
  {
    id: 33,
    title: 'Corporate fleet presentation',
    start: '2026-02-24T09:00:00',
    end: '2026-02-24T10:30:00',
    type: 'meeting',
    customer: 'Auto Roma S.p.A.',
    dealership: 'Customer HQ - Paris',
    assignee: 'Sara Marino',
    assigneeId: 2,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 34,
    title: 'S60 test drive - Matteo Bianchi',
    start: '2026-02-24T11:00:00',
    end: '2026-02-24T12:00:00',
    type: 'test-drive',
    customer: 'Matteo Bianchi',
    customerId: 3,
    vehicle: 'Volvo S60',
    dealership: 'Showroom test drive',
    assignee: 'Natalia Santoro',
    assigneeId: 4,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 35,
    title: 'Dealership visit - Sofia Bellini',
    start: '2026-02-25T09:30:00',
    end: '2026-02-25T10:30:00',
    type: 'appointment',
    customer: 'Sofia Bellini',
    customerId: 6,
    dealership: 'Showroom desk 2',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 36,
    title: 'Recall - airbag replacement',
    start: '2026-02-25T11:00:00',
    end: '2026-02-25T12:00:00',
    type: 'recall',
    vehicle: 'BMW iX',
    dealership: 'Service bay 1',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    team: 'Service',
    status: 'confirmed',
  },
  {
    id: 37,
    title: 'XC90 family test drive',
    start: '2026-02-26T09:00:00',
    end: '2026-02-26T10:00:00',
    type: 'test-drive',
    customer: 'Oliviero Bruno',
    customerId: 5,
    vehicle: 'Volvo XC90',
    dealership: 'Showroom test drive',
    assignee: 'Sara Marino',
    assigneeId: 2,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 38,
    title: 'Monthly results review',
    start: '2026-02-27T09:00:00',
    end: '2026-02-27T10:00:00',
    type: 'meeting',
    dealership: 'Meeting room B',
    assignee: 'Sara Marino',
    assigneeId: 2,
    team: 'General',
    status: 'confirmed',
  },
  {
    id: 39,
    title: 'On-site visit - partner workshop',
    start: '2026-02-27T10:30:00',
    end: '2026-02-27T12:00:00',
    type: 'offsite',
    customer: 'Officina Bianchi',
    dealership: 'Partner workshop',
    assignee: 'Natalia Santoro',
    assigneeId: 4,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 40,
    title: 'Saturday showroom',
    start: '2026-02-28T11:00:00',
    end: '2026-02-28T12:00:00',
    type: 'appointment',
    customer: 'Elisa Costa',
    customerId: 21,
    dealership: 'Showroom desk 1',
    assignee: 'Matteo Greco',
    assigneeId: 1,
    team: 'Sales (New)',
    status: 'confirmed',
  },
  {
    id: 41,
    title: 'Service recall - coolant',
    start: '2026-03-01T09:30:00',
    end: '2026-03-01T10:30:00',
    type: 'recall',
    vehicle: 'Mercedes EQS',
    dealership: 'Service bay 2',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    team: 'Service',
    status: 'confirmed',
  },
  {
    id: 27,
    title: 'Test Drive - Nina Benedetti',
    start: (() => {
      const date = new Date()
      date.setDate(date.getDate() + 2)
      date.setHours(16, 0, 0, 0)
      return date.toISOString()
    })(),
    end: (() => {
      const date = new Date()
      date.setDate(date.getDate() + 2)
      date.setHours(17, 0, 0, 0)
      return date.toISOString()
    })(),
    type: 'test-drive',
    customer: 'Nina Benedetti',
    customerId: 3,
    opportunityId: 2,
    vehicle: 'Audi A3',
    assignee: 'Matteo Alpino',
    assigneeId: 1,
    dealership: 'Frankfurt',
    team: 'Sales (New)',
    status: 'confirmed',
    noShowCount: 0,
    previousAppointmentId: null
  },
  // Giulia Conti (customer 4 / lead 4) — activity timeline calendar bookings
  {
    id: 42,
    title: 'Test Drive - Giulia Conti',
    start: createDateTimeOffset(4, 13, 0),
    end: createDateTimeOffset(4, 14, 0),
    type: 'test-drive',
    customer: 'Giulia Conti',
    customerId: 4,
    leadId: 4,
    vehicle: 'Toyota RAV4',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Bologna',
    team: 'Sales (New)',
    status: 'confirmed',
    bookedAt: createHourOffset(-52),
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 43,
    title: 'Dealership Visit - Giulia Conti',
    start: createDateTimeOffset(2, 13, 0),
    end: createDateTimeOffset(2, 14, 0),
    type: 'appointment',
    customer: 'Giulia Conti',
    customerId: 4,
    leadId: 4,
    vehicle: 'Toyota RAV4',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Bologna',
    team: 'Sales (New)',
    status: 'confirmed',
    bookedAt: createHourOffset(-29),
    noShowCount: 0,
    previousAppointmentId: null
  },
  {
    id: 44,
    title: 'Follow-up Call - Giulia Conti',
    start: createDateTimeOffset(3, 10, 0),
    end: createDateTimeOffset(3, 10, 30),
    type: 'call',
    customer: 'Giulia Conti',
    customerId: 4,
    leadId: 4,
    vehicle: 'Toyota RAV4',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Bologna',
    team: 'BDC',
    status: 'pending_confirmation',
    bookedAt: createHourOffset(-1.1),
    noShowCount: 0
  },
  {
    id: 45,
    title: 'Recall - Giulia Conti',
    start: createDateTimeOffset(6, 9, 0),
    end: createDateTimeOffset(6, 10, 0),
    type: 'recall',
    customer: 'Giulia Conti',
    customerId: 4,
    leadId: 4,
    vehicle: 'Toyota RAV4',
    assignee: 'Davide Rinaldi',
    assigneeId: 3,
    dealership: 'Bologna',
    team: 'Service',
    status: 'confirmed',
    bookedAt: createHourOffset(-0.85),
    noShowCount: 0
  }
]

